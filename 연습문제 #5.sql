...
INSERT INTO `Departments` VALUES (31,'동계학과','051-510-1031');
...
INSERT INTO `Lectures` VALUES ('101001','P10101','대학 글쓰기',2,10,'본103');
...
#문제3
SELECT a.`lecNo`,`lecName`,`proName`,`proHp`
FROM `Lectures` AS a
JOIN `Professors` AS b ON a.proNo=b.proNo;

#문제4
SELECT `lecNo`,`lecName`,`proHp`,`proName`,c.`depNo`,`depName`
FROM `Lectures` AS a
JOIN `Professors` AS b ON a.proNo=b.proNo
JOIN `Departments` AS c ON b.depNo=c.depNo;

#문제5
SELECT `stdNo`,`stdName`,`lecNo`,`lecName`,a.`proNo`,`proName`
FROM `Lectures` AS a
