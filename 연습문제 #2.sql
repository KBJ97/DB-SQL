...
#실습2-15
SELECT SUM(`t_amount`) '입금총합', AVG(`t_amount`) '입금평균' FROM `bank_transaction` WHERE `t_dist`=1;

#실습2-16
SELECT MAX(`t_amount`) '큰 금액', MIN(`t_amount`) '작은 금액' FROM `bank_transaction` WHERE `t_dist`=2;
...
WHERE `t_dist` = 1 GROUP BY `t_a_no` ORDER BY `t_a_no`; 
...
ON a.a_c_no = b.c_no WHERE `a_c_no`=`c_no`;
