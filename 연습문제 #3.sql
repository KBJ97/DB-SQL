...
JOIN `Register` AS b on b.regStdNo = a.stdNo WHERE `regLecNo`=349;
...
ON a.regLecNo = b.lecName;
#실습3-24
select COUNT(*) AS `사회복지 마케팅 수강 시청건수`,
AVG (`regTotalScore`) AS `사회복지 마케팅 평균`
FROM `Register` AS a JOIN `Lecture` AS b ON a.regLecNo = b.lecNo WHERE `regLecNo` = 349 ;

#실습3-25
SELECT `regStdNo`,`lecName` FROM `Register` AS a JOIN `Lecture` AS b 
